...
methods: {
  ...
  async checkDeckId() {
    ...
    const response = await axios.get(`http://localhost:3000/api/cardPDF?input=${this.deckId}`);
  ...
  async exportToPDF() {
      ...
      try {
        const response = await fetch(`http://localhost:3000/api/cardPDF?input=${this.deckId}`);
